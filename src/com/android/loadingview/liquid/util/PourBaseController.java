...
public abstract class PourBaseController extends BaseController {
	...
	void generateBubble(float x, float y) {
		...
		Bubble.BubbleGenerator generator = new Bubble.BubbleGenerator(x, y).with(getCheckView())
				.generateBubbleX(x, radius * 0.5f, pourStrokeWidth * 0.5f).generateBubbleY(y, radius)
				.generateRadius(radius * 0.2f).generateDuration(1500, 500);
