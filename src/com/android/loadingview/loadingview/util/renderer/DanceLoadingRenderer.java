package com.android.loadingview.loadingview.util.renderer;

import com.android.loadingview.loadingview.util.LoadingRenderer;
...
public class DanceLoadingRenderer extends LoadingRenderer {
	...
	// 1: the coordinate x from small to large; -1: the coordinate x from large
	// to small
	...
	private void init(Context context) {
		...
		mDanceBallRadius = dip2px(context, DEFAULT_DANCE_BALL_RADIUS);
	...
	protected void draw(Canvas canvas, Rect bounds) {
		...
		if (mRotation != 0) {
			...
			mPaint.setStyle(Paint.Style.STROKE);
			// strokeWidth / 2.0f + mStrokeWidth / 2.0f is the center of the
			// inter circle width
			mTempBounds.inset(centerRingWidth / 2.0f + mStrokeWidth / 2.0f,
					centerRingWidth / 2.0f + mStrokeWidth / 2.0f);
	...
	protected void computeRender(float renderProgress) {
		...
		if (renderProgress <= BALL_FORWARD_END_ENTER_DURATION_OFFSET
			...
			final float ballForwardEnterProgress = (renderProgress - BALL_FORWARD_START_ENTER_DURATION_OFFSET)
					/ (BALL_FORWARD_END_ENTER_DURATION_OFFSET - BALL_FORWARD_START_ENTER_DURATION_OFFSET);
			...
			for (int i = 0; i < NUM_POINTS; i++) {
				...
				// progress[-1, 1]
				float progress = (ACCELERATE_INTERPOLATOR.getInterpolation(ballForwardEnterProgress) / 2.0f - 0.5f)
						* 2.0f * DIRECTION[i];
		...
		if (renderProgress <= RING_FORWARD_END_ROTATE_DURATION_OFFSET
			...
			final float forwardRotateProgress = (renderProgress - RING_FORWARD_START_ROTATE_DURATION_OFFSET)
					/ (RING_FORWARD_END_ROTATE_DURATION_OFFSET - RING_FORWARD_START_ROTATE_DURATION_OFFSET);
		...
		if (renderProgress <= CENTER_CIRCLE_FORWARD_END_SCALE_DURATION_OFFSET
			...
			if (centerCircleScaleProgress <= 0.5f) {
				...
				mScale = 1.2f
						- ACCELERATE_INTERPOLATOR.getInterpolation((centerCircleScaleProgress - 0.5f) * 2.0f) * 0.2f;
			}
		...
		if (renderProgress <= BALL_FORWARD_END_EXIT_DURATION_OFFSET
			...
			final float ballForwardExitProgress = (renderProgress - BALL_FORWARD_START_EXIT_DURATION_OFFSET)
					/ (BALL_FORWARD_END_EXIT_DURATION_OFFSET - BALL_FORWARD_START_EXIT_DURATION_OFFSET);
			mShapeChangeHeight = (ballForwardExitProgress - 0.5f) * mDanceBallRadius / 2.0f;
			mShapeChangeWidth = -mShapeChangeHeight;
			for (int i = 0; i < NUM_POINTS; i++) {
				float k = (float) Math.tan((DANCE_START_ANGLE + DANCE_INTERVAL_ANGLE * i) / 360.0f * (2.0f * Math.PI));
				float progress = (DECELERATE_INTERPOLATOR.getInterpolation(ballForwardExitProgress) / 2.0f) * 2.0f
						* DIRECTION[i];
				POINT_X[i] = (float) (radius + progress * (radius / Math.sqrt(Math.pow(k, 2.0f) + 1.0f)));
				POINT_Y[i] = k * (POINT_X[i] - radius);
		...
		if (renderProgress <= RING_REVERSAL_END_ROTATE_DURATION_OFFSET
			...
			float scaledTime = (renderProgress - RING_REVERSAL_START_ROTATE_DURATION_OFFSET)
					/ (RING_REVERSAL_END_ROTATE_DURATION_OFFSET - RING_REVERSAL_START_ROTATE_DURATION_OFFSET);
		...
		if (renderProgress <= BALL_REVERSAL_END_ENTER_DURATION_OFFSET
			...
			final float ballReversalEnterProgress = (renderProgress - BALL_REVERSAL_START_ENTER_DURATION_OFFSET)
					/ (BALL_REVERSAL_END_ENTER_DURATION_OFFSET - BALL_REVERSAL_START_ENTER_DURATION_OFFSET);
			mShapeChangeHeight = (0.5f - ballReversalEnterProgress) * mDanceBallRadius / 2.0f;
			mShapeChangeWidth = -mShapeChangeHeight;

			for (int i = 0; i < NUM_POINTS; i++) {
				float k = (float) Math.tan((DANCE_START_ANGLE + DANCE_INTERVAL_ANGLE * i) / 360.0f * (2.0f * Math.PI));
				float progress = (0.5f - ACCELERATE_INTERPOLATOR.getInterpolation(ballReversalEnterProgress) / 2.0f)
						* 2.0f * DIRECTION[i];
				POINT_X[i] = (float) (radius + progress * (radius / Math.sqrt(Math.pow(k, 2.0f) + 1.0f)));
				POINT_Y[i] = k * (POINT_X[i] - radius);
		...
		if (renderProgress <= CENTER_CIRCLE_REVERSAL_END_SCALE_DURATION_OFFSET
			...
			final float centerCircleScaleProgress = (renderProgress
		...
		if (renderProgress <= BALL_REVERSAL_END_EXIT_DURATION_OFFSET
			...
			final float ballReversalExitProgress = (renderProgress - BALL_REVERSAL_START_EXIT_DURATION_OFFSET)
					/ (BALL_REVERSAL_END_EXIT_DURATION_OFFSET - BALL_REVERSAL_START_EXIT_DURATION_OFFSET);
			mShapeChangeHeight = (ballReversalExitProgress - 0.5f) * mDanceBallRadius / 2.0f;
			mShapeChangeWidth = -mShapeChangeHeight;

			for (int i = 0; i < NUM_POINTS; i++) {
				float k = (float) Math.tan((DANCE_START_ANGLE + DANCE_INTERVAL_ANGLE * i) / 360.0f * (2.0f * Math.PI));
				float progress = (0.0f - DECELERATE_INTERPOLATOR.getInterpolation(ballReversalExitProgress) / 2.0f)
						* 2.0f * DIRECTION[i];
				POINT_X[i] = (float) (radius + progress * (radius / Math.sqrt(Math.pow(k, 2.0f) + 1.0f)));
				POINT_Y[i] = k * (POINT_X[i] - radius);
